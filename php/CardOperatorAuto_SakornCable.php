<?php 
	...
	$query_str = mssql_query(" select top 50 'SakornCable' as DB,UserID,RowOrder,CardNO,IsOpenCard,IsUpdateCASAlready from SakornCable.dbo.CustomerCardLog where IsUpdateCASAlready = 0 order by RowOrder asc ");
	...
	while ( $result = mssql_fetch_array($query_str) ) {
		...
		mssql_query(" update SakornCable.dbo.CustomerCardLog set IsUpdateCASAlready = 1 where CardNO = '".$result["CardNO"]."' ");
